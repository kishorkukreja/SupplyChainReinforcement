...
def visualize_transitions(transitions, T=26, warehouse_num=2):
    ...
    for w in range(warehouse_num):
        ...
        plt.plot(range(T), list(map(lambda s: s.warehouse_stock[w], state_trace)), c='purple', alpha=0.5)

    prepare_metric_plot(plots_n, 4, "Production")
    plt.plot(range(T), list(map(lambda a: a.production_level, action_trace)), c='blue', alpha=0.5)

    for w in range(warehouse_num):
        prepare_metric_plot(plots_n, 5 + w, f"Shipment,\n WH {w + 1}")
        plt.plot(range(T), list(map(lambda a: a.shippings_to_warehouses[w], action_trace)), c='blue', alpha=0.5)
