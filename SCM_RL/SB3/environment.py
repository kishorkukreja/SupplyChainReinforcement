...
DEMAND_DATA_PATH = r"C:\Users\kishore.kukreja\Desktop\Inventory Policy Two Echelon\SCM-RL\data\demand.csv"
NUM_WAREHOUSES = 3
EPISODE_DURATION = 30
MAX_DEMAND = 10
UNIT_PRICE = 100
...
class SupplyChainEnvironment(object):
    ...
    def step(self, state: State, action: Action, log=True):
        ...
        if log:
            ...
            log_file = './results/output/demand/demands-'+str(dt_string)+'.csv'
            csvfilename = os.path.join(dirname, 'results','transitions','RL',log_file)
        ...
        if log:
            ...
            # dd/mm/YY H:M:S
            dt_string = now.strftime("%Y-%m-%d-%H-%M")
            log_file = './results/output/costs/Costs-'+str(dt_string)+'.csv'
            csvfilename = os.path.join(dirname, 'results','transitions','RL',log_file)
...
class SimpleSupplyChain(gym.Env):
    ...
    def reset(self):
        ...
        print(self.supply_chain.warehouse_num)
