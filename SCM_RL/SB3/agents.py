...
def train_td3(timesteps=5e5, net_architecture=None):
    ...
    file_name = f"./results/model/TD3/td3_{int(time.time())}"
...
def train_ddpg(timesteps=5e5):
    ...
    file_name = f"./results/model/DDPG/ddpg_{int(time.time())}"
    agent.save(file_name)
...
def test_agent(agent, initial_state,log=False, num_episodes=10):
    env = SimpleSupplyChain()
    print('starts here')
    ...
    for episode in range(num_episodes):
        ...
        while not done:
            ...
            if log:
                transitions.append(
                    [episode, t, *values, *action, reward, total_reward])
    ...
    if log:
        whstock=['warehouse_'+str(i) for i in range(0,num)]
        ...
        df = pd.DataFrame(transitions, columns=columns_names)
        ...
        df.to_csv(
            f"./results/output/transitions/RL/transitions_{num_episodes}_{int(time.time())}.csv", index=False)
