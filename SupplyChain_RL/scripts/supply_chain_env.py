...
class SupplyChainEnv:
    def __init__(self, con_mat, price, k_pr, k_st, k_pe, k_tr, lead_time, st_max, de_hist_len, zeta):
        ...
        count = 0
        for listElem in self.con_mat:
            count += len(listElem)    
        self.action_dim =count + self.factory_num
    ...
    def step(self, action_f, action_w, demand):
        ...
        s_f_next = [0] * self.factory_num #[[0] for count in range(self.factory_num)]
        s_w_next = [0] * self.warehouse_num #[[0] for count in range(self.warehouse_num)]
        for i in range(self.factory_num):
            sum_a_j = 0
            for j in range(self.warehouse_num):
        ...
        de_hist = [[0]*self.de_hist_len for count in range(self.warehouse_num)]
        #[[0]*self.de_hist_len]*self.warehouse_num 
        for j in range(self.warehouse_num):#0,1
            for k in range(self.de_hist_len):#0,1,2,3
